...
\draw[fill=black!20!white, opacity=0.5] (0,0)--(1.4142,1)--(0,2)--(-1.4142,1)--cycle;
\draw[fill=black] (0,2) circle (0.05);

\draw[fill=blue!20!white, opacity=0.5] (0,0)--(1.4142 + 2*1,1.4142 + 1)--(0+2*2,0+2)--(-1.4142 + 2*1,-1.4142 + 1)--cycle;
\draw[fill=black] (0+2*2,0+2) circle (0.05);

\draw[->] (0,0)--(3,0);
\draw[->] (0,0)--(0,3);
\end{tikzpicture} \\
Next can we write down the rectangles so that the matrix map acts perfectly?  Perhaps if we write the equation over:
$$ ( 1 + \sqrt{2})^2 = 2 \times (1 + \sqrt{2}) + 1 $$
and maybe it will be easier if I use the diagonal matrix (and we can slant it later):
$$ \left[ \begin{array}{cc} \sqrt{2}+1 & 0 \\ 0 &  \sqrt{2} - 1\end{array} \right] $$
and perhaps we should act on the torus $[0,1] \times [0,1]$ 
$$ (x,y) \mapsto \Big( \{(\sqrt{2}+1) \times x \}, \{(\sqrt{2}-1) \times y \} \Big) $$
Constructing the actual Markov partition seems to be a bit of an art:
\newpage

\noindent There's arrows everywhere in this thing.  Still no good answer,\dots \\ \\
...
\draw[fill=black!25!white, opacity=0.5] (0,0)--(1,0)--(1,1)--(0,1)--cycle;
...
\node at (1,-0.15) {$1$};
%\node at (2,-0.15) {$1$};
\node at (2.414,-0.15) {$1+\sqrt{2}$};
\node at (-0.15,1) {$1$};

\draw[fill=black!25!white, opacity=0.25] (0,0)--(1,0)--(1,1)--(0,1)--cycle;
\draw[fill=black!10!white, opacity=0.25] (1,0)--(2,0)--(2,1)--(1,1)--cycle;
\draw[fill=black!25!white, opacity=0.25] (2,0)--(3,0)--(3,1)--(2,1)--cycle;
\draw[fill=black!25!white, opacity=0.5] (0,0)--(2.414,0)--(2.414,0.414)--(0,0.414)--cycle;
\draw[fill=black] (2.414, 0) circle (0.025);
\end{scope}
